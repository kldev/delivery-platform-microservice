...
class ConfirmDeliveryRestTest : BaseRestIntegrationTest() {
    ...
    fun `should confirm delivery`() {
        ...
        restTestClient
            .put()
            .uri(url("/api/deliveries/$deliveryId/confirm"))
            .exchange()
            .expectStatus()
            .isNoContent
        ...
        assertThat(delivery.get().status)
            .isEqualTo(DeliveryStatusJpa.CONFIRMED)
    }

    @Test
    fun `should publish delivery confirmed event`() {
        val deliveryId = deliveryDatabaseFixture.create()
        ...
        await()
            ...
            .untilAsserted {
                assertThat(eventPublisher.events())
                    .anyMatch {
                        it.eventType == "delivery.confirmed"
                    }
            }
    }
}